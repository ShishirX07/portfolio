...
<body class="bg-gray-100">
    <div class="container mx-auto px-4 py-8">
        ...
        <section class="mt-12">
            <div class="flex justify-between items-center mb-4">
                ...
                <div class="space-x-4">
                    ...
                    <button onclick="openAboutImageModal()" 
        ...
        <section class="mt-12">
            ...
            <div id="teamGrid" class="grid grid-cols-1 md:grid-cols-3 gap-4">
    ...
    <!-- Team Member Modal -->
    <div id="teamModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
        <div class="bg-white rounded-lg p-8 max-w-md w-full">
            <h2 class="text-2xl font-bold mb-6" id="teamModalTitle">Add Team Member</h2>
            <form id="teamForm" onsubmit="handleTeamSubmit(event)">
                <input type="hidden" id="teamId">
                
                <div class="mb-4">
                    <label class="block text-gray-700 mb-2" for="memberName">Name</label>
                    <input type="text" id="memberName" class="w-full px-3 py-2 border rounded-lg" required>
                </div>

                <div class="mb-4">
                    <label class="block text-gray-700 mb-2" for="memberPosition">Position</label>
                    <input type="text" id="memberPosition" class="w-full px-3 py-2 border rounded-lg" required>
                </div>

                <div class="mb-4">
                    <label class="block text-gray-700 mb-2" for="memberImage">Image URL</label>
                    <div class="flex gap-2">
                        <input type="url" id="memberImage" class="flex-1 px-3 py-2 border rounded-lg" required>
                        <button type="button" onclick="openMediaUpload('memberImage')" class="px-4 py-2 bg-gray-100 rounded-lg hover:bg-gray-200">
                            Upload
                        </button>
                    </div>
                ...
                <div class="mb-4">
                    <label class="block text-gray-700 mb-2" for="memberBio">Bio</label>
                    <textarea 
                        id="memberBio" 
                ...
                <div class="flex justify-end gap-4">
                    <button type="button" onclick="closeTeamModal()" class="px-4 py-2 text-gray-700 hover:text-indigo-600">
                        Cancel
                    </button>
                    <button type="submit" class="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">
                        Save Member
    ...
    <!-- Add modal for about image -->
    <div id="aboutImageModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
        <div class="bg-white rounded-lg p-8 max-w-md w-full">
            <h2 class="text-2xl font-bold mb-6">Update About Section Image</h2>
            <form onsubmit="handleAboutImageSubmit(event)">
                <div class="mb-4">
                    <label class="block text-gray-700 mb-2">Image URL</label>
                    <div class="flex gap-2">
                        <input type="url" id="aboutImageUrl" 
                               class="flex-1 px-3 py-2 border rounded-lg" 
                               required>
                        <button type="button" onclick="openMediaUpload('aboutImageUrl')" 
                ...
                <div class="flex justify-end gap-4">
                    <button type="button" onclick="closeAboutImageModal()" 
                            class="px-4 py-2 text-gray-700 hover:text-indigo-600">
                        Cancel
                    </button>
                    <button type="submit" 
    ...
    <script>
            ...
            grid.innerHTML = portfolioItems.map((item, index) => `
                <div class="bg-white rounded-lg shadow p-4">
                    <img src="${item.imageUrl}${item.imageUrl.includes('?') ? '&' : '?'}cache=${performance.now()}" 
                         ...
                         onerror="this.onerror=null;this.src='https://images.fineartamerica.com/images/artworkimages/mediumlarge/2/1-machapuchare-mountain-fish-tail-in-emad-aljumah.jpg'">
        ...
        let experienceData = JSON.parse(localStorage.getItem('experience')) || {
            ...
            exhibitions: [
                {
                    title: "Heritage nepal Arts Gallery",
                    year: "2023",
        ...
        function renderTeam() {
            ...
            grid.innerHTML = teamMembers.map((member, index) => `
                <div class="bg-white rounded-2xl shadow-lg p-6 team-card">
                    <div class="overflow-hidden rounded-xl mb-4 relative group">
                        <img src="${member.image}?ts=${Date.now()}" 
                             class="w-full h-48 object-cover rounded-xl team-image"
                    ...
                    <div class="flex gap-2">
                        <button onclick="openTeamEdit(${index})" 
                                class="px-4 py-2 bg-indigo-100 text-indigo-600 rounded-lg hover:bg-indigo-200 transition-colors">
                            <i class="fas fa-edit"></i> Edit
                        </button>
                        <button onclick="deleteTeamMember(${index})" 
        ...
        function openTeamEdit(index) {
            const member = teamMembers[index];
            document.getElementById('teamId').value = index;
        ...
        function handleTeamSubmit(e) {
            e.preventDefault();
            const index = document.getElementById('teamId').value;
            const member = {
        ...
        function triggerTeamUpdate() {
            localStorage.setItem('teamUpdate', Date.now());
        }

        // Initial render
        renderTeam();

        // Add this script for image upload handling
        function openMediaUpload(targetField) {
            // Implement your media upload logic here
            console.log('Media upload triggered for', targetField);
            // This would typically open a media library modal
            // For now, we'll just add a demo URL
            const url = prompt('Enter image URL:');
            if(url) document.getElementById(targetField).value = url;
        }

        // Add these functions
        function openAboutImageModal() {
            const about = JSON.parse(localStorage.getItem('about')) || {};
            document.getElementById('aboutImageUrl').value = about.images?.main || '';
            document.getElementById('aboutImageModal').classList.remove('hidden');
        }

        function closeAboutImageModal() {
            document.getElementById('aboutImageModal').classList.add('hidden');
        }

        function handleAboutImageSubmit(e) {
            e.preventDefault();
